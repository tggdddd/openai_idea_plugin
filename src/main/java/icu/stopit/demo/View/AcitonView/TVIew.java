package icu.stopit.demo.View.AcitonView;
...
import icu.stopit.demo.View.Setting.SettingPanel;
...
public class TVIew extends JPanel {
    ...
    public TVIew() {
        ...
        setting.addActionListener(
                e -> ShowSettingsUtil.getInstance().editConfigurable(null, SettingPanel.DISPLAY_NAME)
        );
        send.addActionListener(e ->
                new ActionThread(self).start()
        );
    ...
    private void $$$setupUI$$$() {
        ...
        scrollPane2.setViewportView(question);
    ...
    void showImageList(List<Image> images) throws Exception {
...
class ActionThread extends Thread {
    ...
    public ActionThread(TVIew view) {
        this.tvIew = view;
    }
    ...
    public void run() {
        synchronized (tvIew.object) {
            tvIew.send.setEnabled(false);
            String text = tvIew.question.getText();
            StringBuilder stringBuilder = new StringBuilder();
            try {
                String selectModel = (String) tvIew.comboBox1.getSelectedItem();
                if (selectModel.equals(tvIew.TEXT_GENERATION)) {
                ...
                } else if (selectModel.equals(tvIew.TEXT_EDIT)) {
                    String instruction = tvIew.answer.getText();
                ...
                } else if (selectModel.equals(tvIew.PICTURE_GENERATION)) {
                    if (text == null || text.isEmpty()) {
                        throw new Exception("输入不能为空");
                    }
                ...
                } else if (selectModel.equals(tvIew.IMAGE_VARIATION)) {
                    ...
                    assert images != null;
                    Utils.log(String.format("相似图片：来源【%s】\n结果:\n【%s】\n", tvIew.uploadImageFile.getAbsolutePath(), images.stream().map(Image::getUrl).reduce((a, b) -> a + "\n" + b).get()));
                } else if (selectModel.equals(tvIew.IAMGE_EDIT)) {
                    if (text == null || text.isEmpty()) {
                        throw new Exception("输入不能为空");
                    }
                    ...
                    assert images != null;
                    Utils.log(String.format("图片编辑：来源【%s】，\n掩码【%s】\n描述【%s】\n结果:\n【%s】\n", tvIew.uploadImageFile.getAbsolutePath(), tvIew.upLoadImageMask.getAbsolutePath(), text, images.stream().map(Image::getUrl).reduce((a, b) -> a + "\n" + b).get()));
                }
                ...
                tvIew.answer.setText(Utils.trimDuplicateSpace(stringBuilder.toString()));
                tvIew.send.setEnabled(true);
            }
        }
    }
}
